...
var utility = function() {
    return {
        ...
        * @return {string} certificiate in string format
        */
        parseCer: function parseCer(cert){
            return cert.replace(/\n/g, '').replace(/\r/g, '').replace('-----BEGIN CERTIFICATE-----', '').replace('-----END CERTIFICATE-----', '');
        },
        /**
        ...
        */
        readPrivateKeyFromFile: function readPrivateKeyFromFile(keyFile, passphrase, isOutputString){
            try {
                ...
                var decryptedPrivKey = pki.decryptRsaPrivateKey(fs.readFileSync(keyFile), passphrase);
                var privKeyPem = pki.privateKeyToPem(decryptedPrivKey);
                var pemString = this.convertToString(privKeyPem, isOutputString);
                return pemString;
            } catch (e) {
                // check if the keyfile has passphrase and if it's correct
                console.log('fatal: ', e);
                throw new Error('read private key file error');
            }
        },
        /**
